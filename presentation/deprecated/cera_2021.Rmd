...
.left[
.footnote[
Graphic by[Boom](https://www.rawpixel.com/image/3112764/free-photo-image-marble-background-aesthetic)
]
]
...
- Director California System of Support
]
...
- Director Differentiated Assistance Year 3
]
...
To address inequity, school funding formulas directed more monies to districts that serve larger percentages of historically marginalized student groups.
...
Steve. There are a number of reasons why we decided to explore the effects of differentiated assistance. 
...
.pull-right[
```{r, ref.label= "CA-map", out.width="90%"}
```
]
...
Steve. When the state operationalized DA in the 2016-17 school year, approximately 218 districts of our 1,390 districts and charters were eligible for assistance based on results from the California Schools Dashboard. 

Each of these dots represents a school district across the state that was eligible for assistance from the largest Los Angelos Unified to the smallest Redding School District.
...
Steve.DA is state assistance provided to districts that fail to meet the performance criteria established by the State Board of Education (SBE). 
...
.left-column[
<br>Most accountability interventions have limited effects on student learning outcomes. The greatest changes in student achievement occurred when states *invested sufficient resources* into district capacity and when districts and schools *implemented interventions effectively*.
]

```{r fig.show="hide", include=FALSE, ggdotchart, eval=TRUE}
knitr::include_graphics("img/figure11.ggdotchart.png")
```

.right-column[
```{r, ref.label= "ggdotchart", fig.align='center', out.width="100%"}
```
]

???

Steve.Here we have illustrated the effects of some accountability initiatives nationwide. 
...
Shannon.We were interested in learning answers to two major questions. The first question was how assistance changed student achievement. That is to say, in 2016-17, when DA turned on officially for some districts, how did student achievement change?
...
Shannon.Our second question focused on the achievement gap. We were interested in changes occurring to the achievement gap after the DA policy turned on for districts.
...
knitr::include_graphics("img/descript2.png")
```

.right-column[
```{r, ref.label= "descript", fig.align='center', out.width="85%"}
```
]
...
???
Shannon.We  compared differentiated assistance districts to a subset of 270 
...
.left-column[
<br>DD makes use of longitudinal data from treatment and control groups to obtain a counterfactual to estimate a causal effect. DD estimates the effect of a specific intervention by comparing the changes in outcomes over time between a population that is enrolled in a program and a population that is not.
]

```{r fig.show="hide", include=FALSE, did-explain, eval=TRUE}
knitr::include_graphics("img/image009.png")
```

.right-column[
```{r, ref.label= "did-explain", fig.align='center', out.width="100%"}
```
]
...
<br>These effects represent the mean difference between the assistance and non-assistance groups in standard deviation units. For both years and across both ELA and math the mean difference in the assistance districts compared to the non-assistance districts was no different from zero.
]
```{r fig.show="hide", include=FALSE, achievement, eval=TRUE}
knitr::include_graphics("img/image001.png")
```

.right-column[
```{r, ref.label= "achievement", fig.align='center', out.width="100%"}
```
]

???

Shannon.What’s the effect of assistance on achievement?
...
----
.left-column[
<br>The achievement gap between low-income districts receiving assistance relative to other groups decreased modestly after the first year after the policy change in both ELA and math. These effects appear to dissipate in year two. Unfortunately, none of the results are significantly different from zero.
]
```{r fig.show="hide", include=FALSE, achievement-gap, eval=TRUE}
knitr::include_graphics("img/image004.png")
```

.right-column[
```{r, ref.label= "achievement-gap", fig.align='center', out.width="100%"}
```
]

???

Shannon.Effect of assistance on the achievement gap.
...
Dan.We know that English language arts (ELA) and math scores of assistance districts improved fractionally, but not meaningfully, compared to similar districts during the report study and that DA did not significantly reduce the achievement gap between low-income districts receiving assistance compared to other similar districts that did not receive assistance. We also know that having two years of data makes it difficult to measure the effects of differentiated assistance. It may take three to five years to get an accurate assessment.

Sometimes, just being labeled a “low-performing school” leads to a decrease in school performance, regardless of the assistance level. Go back to 11.
...
Dan.Building capacity at the district level and holding a district accountable for improvement might mitigate many of the unintended consequences of accountability that result when schools feel punished for actions beyond their control. I’m sorry to mention Atlanta Public Schools, but the pressure to improve can force people to make bad choices. DA doesn’t focus on a single student test score, so it is less likely to have unintended consequences. However, the overall effects on student achievement might be limited. The effects of systems change may not be as impactful as the impact that can occur at the classroom level.

County offices of education don’t implement differentiated assistance consistently across agencies. 
...
Dan.The research supporting differentiated assistance is sound. It aims to get at the root cause of low student achievement, not just find a Band-aid or temporary solution. It also focuses on district systems, which is the place where the greatest change can happen to improve schools.
...
[Improvement Guide](https://www.sdcoe.net/lls/assessment/Pages/evaluation-reports.aspx)

???

Dan. Link to improvement guide.
---
